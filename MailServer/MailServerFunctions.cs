...
public class MailServerFunctions
{
    ...
    public StandardResponse SendSMTP(LoggerInfo loggerInfo, string username, string password, string fromAddress, string fromAddressReadable, string toAddress, string toAddressReadable, string subject, string bodyText, int timeout, bool includeID, bool includePaymentReceipt, string inReplyTo, ref string myMessageId)
    {
        ...
        try
        {
            ...
            message.MessageId = CalculateMD5Hash(DateTime.Now.ToString("yyyyMMddhhmmssfff")) + inReplyToPartial + GetRandomCharacters(rand, 15) + "@" + settings.OutgoingMessageIdDomainName; //Default is to Imitate the gmail message ID
            ...
            foreach (string s in toSplit)
            {
                ...
                {
                    Logger.WriteDbg(loggerInfo, "Friendly: " + ScrubText(friendly) + ", Email: " + realAddress);
    ...
    public bool RemoveReplyTextFromMessageStartingAtDate(ref string text, string compare)
    {
        ...
        if (pos > 0)
        {
            ...
            {
                if (IsStringSomeTypeOfDateTime(dateTimeStr))
                {
    ...
    private bool IsStringSomeTypeOfDateTime(string s)
    {
        s = s.Trim();
    ...
    {
        return RemoveUselessText(MakeEmailEasierToRead(FixMessageTypos(subjectLine + " " + RemoveReplyTextFromMessage(emailBody).Replace("\r\n", " ").Replace("'", ""))));
    }
    ...
    private string HandleDirectQuestions(string body, ref MailStorage currentMessage, Random rand)
    {
        ...
        if (preProcessedBody.Trim().ToUpper().Contains("INDEED YOU ARE A HUMAN") ||
            ...
            preProcessedBody.Trim().ToUpper().Contains("AUTOMATIC MESSAGE YOU ALWAYS SEND") ||
